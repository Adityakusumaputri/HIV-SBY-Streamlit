...
#Modul shutil menyediakan antarmuka tingkat tinggi untuk operasi file.
#memperoleh informasi terperinci tentang di mana kesalahan
#  KONFIGURASI HALAMAN
# ================================
st.set_page_config(
...
if selected == "Dashboard":
    ...
    st.markdown("""
        <div class="dashboard-header animate-fade-in">
            <div class="header-content">
                ...
                <div>
                    <h1 style='font-size: 28px; font-weight: 700; margin: 0; letter-spacing: -0.5px;'> Dashboard</h1>
    ...
    with st.expander("Pengaturan Tampilan Power BI (Klik jika dashboard tidak muncul)"):
        st.markdown("""
        <small style='color: #64748b;'>
        Jika tampilan kosong atau meminta login, link default mungkin bersifat privat. 
        Silakan masukkan <b>Link Embed Publik</b> dari Power BI.
...
# Halaman input
    ...
    with col1:
        ...
        #menghub kan jenis file dan file yg di up
        selected_type = st.selectbox("Pilih Jenis Data", list(file_type_map.keys()))
        target_filename = file_type_map[selected_type]
        #Membuka dialog untuk pengguna mengupload file Excel dalam format .xlsx.
        uploaded_file = st.file_uploader("", type=["xlsx"], label_visibility="collapsed")
        #penyimpanan dan membaca file
        if uploaded_file is not None:
            ...
            try:
                ...
                try:
                    ...
                    # data akan disortir berdasarkan tahun (dari yang terbaru ke yang lebih lama) memastikan data ada
                ...
                # ditampilkan dalam bentuk DataFrame untuk dilihat oleh pengguna.
                ...
                with col_btn1:
                    if st.button("Proses ETL", use_container_width=True):
                        with st.spinner("Menjalankan Transformasi Data"):
                            try:
                                # Reload Modul untuk Memastikan Perbaikan Terbaru
                                importlib.reload(tranform)
                                
                                # Mapping Jenis Data yang Dipilih ke Kunci Internal
                                key_map = {
                                ...
                                # Menentukan Kunci untuk Transformasi
                                etl_key = key_map[selected_type]
                                
                                 # Mempersiapkan Data untuk Transformasi
                                data_dict = {etl_key: df.copy()}
                                
                                # Validasi dan Perbaikan Data Umum
                ...
                with col_btn2:
                    if st.button("Upload ke Database", type="primary", use_container_width=True):
                        with st.spinner("Memuat data ke Database MySQL"):
                            #proses up
                            try:
                                # Paksa Muat Ulang Modul untuk menerapkan perbaikan terbaru
                                importlib.reload(load)

                                # Tentukan target spesifik berdasarkan seleksi untuk menghindari pencampuran data.
                                key_map = {
                                    "Jenis Kelamin (Global)": ["jeniskelamin"],
                                    "Jenis Kelamin (Surabaya)": ["jeniskelaminsby"],
                                    "Per Kecamatan": ["perkecamatan"],
                                ...
                                st.toast("Data berhasil masuk Database!")
