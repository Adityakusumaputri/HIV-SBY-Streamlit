...
def run_load_process(target_tables=None):
    ...
    # cursor.execute("DROP DATABASE datawarehouse_hiv")
    ...
    CREATE TABLE IF NOT EXISTS dim_kelompok_umur (
        ...
        kelompok_umur VARCHAR(50) UNIQUE
    )
    """)
    ...
    CREATE TABLE IF NOT EXISTS dim_jenis_kelamin (
        ...
        jenis_kelamin VARCHAR(20) UNIQUE
    )
    """)
    ...
    CREATE TABLE IF NOT EXISTS dim_status_pasien (
        ...
        status_pasien VARCHAR(50) UNIQUE
    )
    """)
    ...
    #  memasukan data ke dimensi
    #Sintaks %s adalah tempat untuk parameter yang akan dimasukkan
    # dim_waktu
    ...
    # perkecamatan.csv (ekstrak tahun dari nama kolom)
    ...
    # file lain yang memiliki kolom Tahun
            ...
            for _, r in df.iterrows():
                ...
                for col in df.columns:
                    if "Temuan" in col:
                        ...
                        # Fungsi helper untuk clean dan convert nilai membersihkan dan mengonversi nilai agar bisa diproses
                        def clean_and_convert(val):
                            if pd.isna(val) or val == '':
                                return 0
                            # Convert ke string dan bersihkan karakter tab, newline, spasi ganda
                            ...
                            try:
                                ...
                                print(f"Warning: Tidak bisa convert '{val}' (cleaned: '{val_str}') ke angka, menggunakan 0")
                                return 0

                        temuan = clean_and_convert(val_temuan)
                        art = clean_and_convert(val_art)
